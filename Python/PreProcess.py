...
def LeakDecision (trainFile,testFile,out1,out2,dirDict=None,
    ...
    # First step on test data. Select and save index
    ...
    with open(testFile, newline='') as csvfile:
        ...
        for row in reader :
            ...
            if (reader.line_num%100000==0) :
                print("look-test :",reader.line_num," d1-4 : ",
    ...
    with open(testFile, newline='') as csvfile:
        ...
        for row in reader :
            ...
            if ((SDI!="") and (HCountry!="") and (HMarket!="")) :
                 ...
                 key = hash("-".join(["1",SDI,"2",HCountry,"3",HMarket]))
                 if key in d2 :
                     if (len(d2[key])>0) : mark += 2; out = out + d2.get(key)

            if ((SDI!="")) :
                 key = (SDI)
                 if key in d3 :
                     if (len(d3[key])>0) : mark += 4; out = out + d3.get(key)

            if ((HCountry!="")) :
                 key = (HCountry)
                 if key in d4 :
                     if (len(d4[key])>0) : mark += 8; out = out + d4.get(key)
                 
            if (len(out)<5) : out  = (out+top5x0); mark += 16;
            out = out[:5]
            if (debug and reader.line_num>600) : break;
            out = (" ".join(out)).strip()
